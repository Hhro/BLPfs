...
void Dispatcher::ExecuteEntryVM() {
    ...
    if (pid < 0) {
        ...
        while (1) {
            if (this->HandleCommands(sockets[0], 1) != 0)
                break;
        }
    }
}
...
int Dispatcher::HandleCommands(int socket, int ulevel) {
    uint8_t opcode;
    char filename_buf[9] = {0, };

    if (read(socket, &opcode, sizeof(opcode)) != sizeof(opcode))
        return -1;
    
    switch (opcode) {
        case OP_READ:
        {   // suppress compiler complains
            if (read(socket, filename_buf, sizeof(filename_buf) - 1) != sizeof(filename_buf) - 1)
                return -1;
            std::string filename(filename_buf);
            uint32_t pos, len;
            if (read(socket, &pos, sizeof(pos)) != sizeof(pos))
                return -1;
            if (read(socket, &len, sizeof(len)) != sizeof(len))
                return -1;
            char *buf = (char *) malloc(len);
            int ret;
            if ((ret = this->fs_mgr_->ReadFile(filename, pos, len, ulevel, buf)) != -1) {
                // TODO: Change check when https://github.com/Hhro/BLPfs/issues/3 is fixed. 
                len = 0;
            }
